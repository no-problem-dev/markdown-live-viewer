...
1. **複数ディレクトリでの同時起動**
   ...
   - 各インスタンスは異なるポートで動作
   - ポート番号の自動割り当て（または明示的指定）
   - `mdv --port 3001 /path/to/dir1` のような形式でポート指定可能
...
2. **モジュール形式**
   - ESM（ECMAScript Modules）形式での実装
   - CommonJS形式は使用しない
   - 動的importやtop-level awaitを活用
...
2. **フレームワーク・ライブラリ**
   - Expressを基盤とする（または軽量フレームワーク）
   - 既存パッケージ: marked, mermaid等は継続使用
   - 新規依存性は最小限に

3. **デプロイ・配布**
   - npmパッケージとして公開
   - スコープ付きパッケージ名検討: `@markserv/cli` または `markserv-next`

### 後方互換性
1. **既存markservとの関係**
   - 既存のmarkservとの後方互換性は考慮しない
   - メジャーバージョンアップ（v3.0.0以上）として位置付け
   - 移行ガイドの提供
...
- 将来実装機能のマーク追加（LaTeX、行番号、ファイルツリー、サイドバー）
